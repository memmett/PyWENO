...
void
weights_gauss_lobatto003003 (const double *restrict sigma, int n, int ssi, int ssr,
  ...
  for (i = 3; i < n - 3; i++)
    {
      ...
      omega5m = (omega5m) / (acc);
      acc = 0.0;
      omega6 = (+0.3) / ((sigma0 + 1.0e-6) * (sigma0 + 1.0e-6));
      acc = acc + omega6;
      omega7 = (+0.6) / ((sigma1 + 1.0e-6) * (sigma1 + 1.0e-6));
      acc = acc + omega7;
      omega8 = (+0.1) / ((sigma2 + 1.0e-6) * (sigma2 + 1.0e-6));
      acc = acc + omega8;
      omega6 = (omega6) / (acc);
      omega7 = (omega7) / (acc);
      omega8 = (omega8) / (acc);
...
PyObject *
py_weights_gauss_lobatto003003 (PyObject * self, PyObject * args)
{
  double *sigma, *omega;
  ...
  weights_gauss_lobatto003003 (sigma, n, ssi, ssr, omega, wsi, wsl, wsr);
...
void
reconstruct_gauss_lobatto003003 (const double *restrict f, int n, int fsi,
...
PyObject *
py_reconstruct_gauss_lobatto003003 (PyObject * self, PyObject * args)
{
  double *f, *omega, *fr;
  ...
  reconstruct_gauss_lobatto003003 (f, n, fsi, omega, wsi, wsl, wsr, fr, frsi, frsl);
