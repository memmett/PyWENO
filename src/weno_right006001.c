...
void
weights_right006001 (const double *restrict sigma, int n, int ssi, int ssr,
  ...
  for (i = 6; i < n - 6; i++)
    {
      ...
      acc = 0.0;
      omega0 = (+0.012987012987013) / ((sigma0 + 1.0e-6) * (sigma0 + 1.0e-6));
      acc = acc + omega0;
      omega1 = (+0.162337662337662) / ((sigma1 + 1.0e-6) * (sigma1 + 1.0e-6));
      acc = acc + omega1;
      omega2 = (+0.432900432900433) / ((sigma2 + 1.0e-6) * (sigma2 + 1.0e-6));
      acc = acc + omega2;
      omega3 = (+0.324675324675325) / ((sigma3 + 1.0e-6) * (sigma3 + 1.0e-6));
      acc = acc + omega3;
      omega4 = (+0.0649350649350649) / ((sigma4 + 1.0e-6) * (sigma4 + 1.0e-6));
      acc = acc + omega4;
      omega5 = (+0.00216450216450216) / ((sigma5 + 1.0e-6) * (sigma5 + 1.0e-6));
      acc = acc + omega5;
      omega0 = (omega0) / (acc);
      omega1 = (omega1) / (acc);
      omega2 = (omega2) / (acc);
...
PyObject *
py_weights_right006001 (PyObject * self, PyObject * args)
{
  double *sigma, *omega;
  ...
  weights_right006001 (sigma, n, ssi, ssr, omega, wsi, wsl, wsr);
...
void
reconstruct_right006001 (const double *restrict f, int n, int fsi,
			 ...
			 double *restrict fr, int frsi, int frsl)
{
  ...
  for (i = 6; i < n - 6; i++)
	...
	(-0.216666666666667) * (f[(i + 4) * fsi]) + (+0.0333333333333333) * (f[(i + 5) * fsi]);
      fr1 =
	...
	(+0.616666666666667) * (f[(i + 0) * fsi]) + (+0.616666666666667) * (f[(i + 1) * fsi]) +
...
PyObject *
py_reconstruct_right006001 (PyObject * self, PyObject * args)
{
  double *f, *omega, *fr;
  ...
  reconstruct_right006001 (f, n, fsi, omega, wsi, wsl, wsr, fr, frsi, frsl);
