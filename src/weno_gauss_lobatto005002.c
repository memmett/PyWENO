...
void
weights_gauss_lobatto005002 (const double *restrict sigma, int n, int ssi, int ssr,
  ...
  for (i = 5; i < n - 5; i++)
    {
      ...
      omega4 = (omega4) / (acc);
      acc = 0.0;
      omega5 = (+0.0396825396825571) / ((sigma0 + 1.0e-6) * (sigma0 + 1.0e-6));
      acc = acc + omega5;
      omega6 = (+0.3174603174603) / ((sigma1 + 1.0e-6) * (sigma1 + 1.0e-6));
      acc = acc + omega6;
      omega7 = (+0.476190476190482) / ((sigma2 + 1.0e-6) * (sigma2 + 1.0e-6));
      acc = acc + omega7;
      omega8 = (+0.158730158730158) / ((sigma3 + 1.0e-6) * (sigma3 + 1.0e-6));
      acc = acc + omega8;
      omega9 = (+0.00793650793650794) / ((sigma4 + 1.0e-6) * (sigma4 + 1.0e-6));
      acc = acc + omega9;
      omega5 = (omega5) / (acc);
      omega6 = (omega6) / (acc);
      omega7 = (omega7) / (acc);
...
PyObject *
py_weights_gauss_lobatto005002 (PyObject * self, PyObject * args)
{
  double *sigma, *omega;
  ...
  weights_gauss_lobatto005002 (sigma, n, ssi, ssr, omega, wsi, wsl, wsr);
...
void
reconstruct_gauss_lobatto005002 (const double *restrict f, int n, int fsi,
...
PyObject *
py_reconstruct_gauss_lobatto005002 (PyObject * self, PyObject * args)
{
  double *f, *omega, *fr;
  ...
  reconstruct_gauss_lobatto005002 (f, n, fsi, omega, wsi, wsl, wsr, fr, frsi, frsl);
