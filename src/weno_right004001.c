...
void
weights_right004001 (const double *restrict sigma, int n, int ssi, int ssr,
...
PyObject *
py_weights_right004001 (PyObject * self, PyObject * args)
{
  double *sigma, *omega;
  ...
  weights_right004001 (sigma, n, ssi, ssr, omega, wsi, wsl, wsr);
...
void
reconstruct_right004001 (const double *restrict f, int n, int fsi,
			 ...
			 double *restrict fr, int frsi, int frsl)
{
  ...
  for (i = 4; i < n - 4; i++)
	...
	(+0.583333333333333) * (f[(i + 1) * fsi]) + (-0.0833333333333333) * (f[(i + 2) * fsi]);
      fr2 =
	(+0.0833333333333333) * (f[(i - 2) * fsi]) + (-0.416666666666667) * (f[(i - 1) * fsi]) +
...
PyObject *
py_reconstruct_right004001 (PyObject * self, PyObject * args)
{
  double *f, *omega, *fr;
  ...
  reconstruct_right004001 (f, n, fsi, omega, wsi, wsl, wsr, fr, frsi, frsl);
