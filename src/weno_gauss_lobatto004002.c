...
void
weights_gauss_lobatto004002 (const double *restrict sigma, int n, int ssi, int ssr,
  ...
  for (i = 4; i < n - 4; i++)
    {
      ...
      omega3 = (omega3) / (acc);
      acc = 0.0;
      omega4 = (+0.114285714285711) / ((sigma0 + 1.0e-6) * (sigma0 + 1.0e-6));
      acc = acc + omega4;
      omega5 = (+0.514285714285716) / ((sigma1 + 1.0e-6) * (sigma1 + 1.0e-6));
      acc = acc + omega5;
      omega6 = (+0.342857142857143) / ((sigma2 + 1.0e-6) * (sigma2 + 1.0e-6));
      acc = acc + omega6;
      omega7 = (+0.0285714285714286) / ((sigma3 + 1.0e-6) * (sigma3 + 1.0e-6));
      acc = acc + omega7;
      omega4 = (omega4) / (acc);
      omega5 = (omega5) / (acc);
      omega6 = (omega6) / (acc);
...
PyObject *
py_weights_gauss_lobatto004002 (PyObject * self, PyObject * args)
{
  double *sigma, *omega;
  ...
  weights_gauss_lobatto004002 (sigma, n, ssi, ssr, omega, wsi, wsl, wsr);
...
void
reconstruct_gauss_lobatto004002 (const double *restrict f, int n, int fsi,
...
PyObject *
py_reconstruct_gauss_lobatto004002 (PyObject * self, PyObject * args)
{
  double *f, *omega, *fr;
  ...
  reconstruct_gauss_lobatto004002 (f, n, fsi, omega, wsi, wsl, wsr, fr, frsi, frsl);
