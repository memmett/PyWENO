...
void
weights_gauss_lobatto003002 (const double *restrict sigma, int n, int ssi, int ssr,
  ...
  for (i = 3; i < n - 3; i++)
    {
      ...
      omega2 = (omega2) / (acc);
      acc = 0.0;
      omega3 = (+0.3) / ((sigma0 + 1.0e-6) * (sigma0 + 1.0e-6));
      acc = acc + omega3;
      omega4 = (+0.6) / ((sigma1 + 1.0e-6) * (sigma1 + 1.0e-6));
      acc = acc + omega4;
      omega5 = (+0.1) / ((sigma2 + 1.0e-6) * (sigma2 + 1.0e-6));
      acc = acc + omega5;
      omega3 = (omega3) / (acc);
      omega4 = (omega4) / (acc);
      omega5 = (omega5) / (acc);
...
PyObject *
py_weights_gauss_lobatto003002 (PyObject * self, PyObject * args)
{
  double *sigma, *omega;
  ...
  weights_gauss_lobatto003002 (sigma, n, ssi, ssr, omega, wsi, wsl, wsr);
...
void
reconstruct_gauss_lobatto003002 (const double *restrict f, int n, int fsi,
...
PyObject *
py_reconstruct_gauss_lobatto003002 (PyObject * self, PyObject * args)
{
  double *f, *omega, *fr;
  ...
  reconstruct_gauss_lobatto003002 (f, n, fsi, omega, wsi, wsl, wsr, fr, frsi, frsl);
