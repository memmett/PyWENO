...
void
weights_gauss_lobatto006002 (const double *restrict sigma, int n, int ssi, int ssr,
  ...
  for (i = 6; i < n - 6; i++)
    {
      ...
      omega5 = (omega5) / (acc);
      acc = 0.0;
      omega6 = (+0.0129870129869969) / ((sigma0 + 1.0e-6) * (sigma0 + 1.0e-6));
      acc = acc + omega6;
      omega7 = (+0.162337662337667) / ((sigma1 + 1.0e-6) * (sigma1 + 1.0e-6));
      acc = acc + omega7;
      omega8 = (+0.432900432900435) / ((sigma2 + 1.0e-6) * (sigma2 + 1.0e-6));
      acc = acc + omega8;
      omega9 = (+0.324675324675325) / ((sigma3 + 1.0e-6) * (sigma3 + 1.0e-6));
      acc = acc + omega9;
      omega10 = (+0.0649350649350649) / ((sigma4 + 1.0e-6) * (sigma4 + 1.0e-6));
      acc = acc + omega10;
      omega11 = (+0.00216450216450216) / ((sigma5 + 1.0e-6) * (sigma5 + 1.0e-6));
      acc = acc + omega11;
      omega6 = (omega6) / (acc);
      omega7 = (omega7) / (acc);
      omega8 = (omega8) / (acc);
...
PyObject *
py_weights_gauss_lobatto006002 (PyObject * self, PyObject * args)
{
  double *sigma, *omega;
  ...
  weights_gauss_lobatto006002 (sigma, n, ssi, ssr, omega, wsi, wsl, wsr);
...
void
reconstruct_gauss_lobatto006002 (const double *restrict f, int n, int fsi,
...
PyObject *
py_reconstruct_gauss_lobatto006002 (PyObject * self, PyObject * args)
{
  double *f, *omega, *fr;
  ...
  reconstruct_gauss_lobatto006002 (f, n, fsi, omega, wsi, wsl, wsr, fr, frsi, frsl);
